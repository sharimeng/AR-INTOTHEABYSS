...
const createGreetingOverlay = () => {
  // 1. ADD GLASSMORPHISM BACK BUTTON (Top Left)
  const backBtn = document.createElement("a");
  backBtn.href = "../index.html"; // Redirects to index.html
  backBtn.className = "custom-back-btn";
  backBtn.title = "Back";
  // SVG Icon from reference
  backBtn.innerHTML = `
  ...
  // 2. Original Greeting Overlay Code
...
(async () => {
  ...
  anchor.onTargetFound = () => { 
    if (!hasScanned) {
      ...
      // REMOVE THE GLOBAL BACK BUTTON HERE
      const globalBackBtn = document.querySelector('.custom-back-btn');
